...
![*From the Cartoon Guide to Statistics*](images/spread1.png){width=40%} ![](images/spread2.jpg){width=42%}
...
APIs can make a social scientist's life easier by providing an efficient way to collect data. Without an API, researchers might have to resort to manually extracting information from online or writing an ad hoc set of code to "scrape" the information off of websites. However, because researchers cannot control the API, the downside is at any given time, an organization could change or remove API access.

Many researchers have developed corresponding R packages that make it easier to work with APIs. We will explore two of them.
